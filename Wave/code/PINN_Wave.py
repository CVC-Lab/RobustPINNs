...
'''
NYU Benchmark Wave
https://archive.nyu.edu/bitstream/2451/63316/2/nn-benchmark-data-wave.tar.gz

This script uses the first trajectory among 50 trajectories of the training set

Data channels
---
q : (Nt,Np) = position/height of the wave at each grid point
p : (Nt,Np) = vertical velocity of the wave at each grid point
dqdt : (Nt,Np)
dpdt : (Nt,Np)
t : (Nt)
---
"wave_speed": 0.1
Nt = 10205 = the number of time steps in the trajectory (5 seconds)
"time_step_size": 0.00049
Np = 125  = the number of points in the spatial grid [0,1)

Note that we will use u to denote p and v to denote q.
dq/dt = p              -> dv/dt = u
dp/dt = c^2 d^2q/dx^2  -> du/dt = c^2 d^2v/dx^2
'''

npz = np.load("../dataset/trajectories.npz")
p1, q1, dqdt1, dpdt1, t1 = npz[npz.files[0]], npz[npz.files[1]], npz[npz.files[2]], npz[npz.files[3]], npz[npz.files[4]] #The first trajectory
t = torch.tensor(t1, dtype = torch.float32).reshape(-1)
x = torch.tensor(np.linspace(0,1,125), dtype = torch.float32).reshape(-1)
Exact_u = torch.tensor(p1.T, dtype = torch.float32)
Exact_v = torch.tensor(q1.T, dtype = torch.float32)
X, T = np.meshgrid(x,t)
...
class PINN(nn.Module):
    ...
    def PhysicsLoss(self, XTGrid):
        ...
        vf_xx = torch.autograd.grad(outputs=vf_x.to(self.device), 
                                    ...
                                    allow_unused=True)[0][:,0]
        '''
        Note that we use u to denote p and v to denote q.
        dq/dt = p              -> dv/dt = u
        dp/dt = c^2 d^2q/dx^2  -> du/dt = c^2 d^2v/dx^2
        '''
        lossf =  torch.mean((vf_t - uf)**2 + (uf_t - c**2 * vf_xx)**2)
        return lossf
...
if __name__ == '__main__':
        ...
        ##LBs = [0.0, 0.0]
        ##UBs = [1.0, 5.0]
        boundaries = (1/NDomains)*np.arange(0,NDomains+1)
        t_domain = [0., 5.]
